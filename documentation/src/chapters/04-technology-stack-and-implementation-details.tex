...
\begin{lstlisting}[language=Erlang, caption={Shared Job Record},label={lst:shared-job-record}]
-record(job, {
  id,
  type,
  priority,
...
\subsection{Swagger UI}\label{subsec:swagger-ui}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{images/jobs-management}

    \caption{API Gateway Job Management endpoints.}
    \label{fig:api-gateway-job-management}
\end{figure}


\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{images/jobs-schemas}

    \caption{API Gateway Job Schemas.}
    \label{fig:api-gateway-job-schemas}
\end{figure}
