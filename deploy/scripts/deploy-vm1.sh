#!/bin/bash
# Deployment script for VM1 (10.2.1.3)

set -e

echo "========================================="
echo "Deploying DistriQueue on VM1 (10.2.1.3)"
echo "========================================="

# Load environment variables
source /opt/distriqueue/config/vm1.env
...
EnvironmentFile=/opt/distriqueue/config/vm1.env
...
cat > /opt/distriqueue/config/application-vm1.yml << EOF
server:
...
ExecStart=/usr/bin/java -jar /opt/distriqueue/bin/distriqueue-api-gateway.jar \
    --spring.config.location=/opt/distriqueue/config/application-vm1.yml
Restart=on-failure
RestartSec=10
